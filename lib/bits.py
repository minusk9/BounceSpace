...
# use math numpy numpy linalg os path play pygame pygame display pygame event pygame image pygame mouse pygame sprite pygame time random sys configparser
import math
import numpy
import numpy.linalg
import os.path
...
class BITS:
	...
	def __init__(self, path, conf):
		...
		self.path = path
		self.ship = play8.screen64_make(self.font[0], play8.bytes_shape(bytes.fromhex("4e 4d 66 66 66 66 4d 4e"), 2))
		return
	...
	def body(self):
		...
		if pygame.mixer:
			pygame.mixer.music.load(os.path.join(self.path, "res", "bounce.mp3"))
	...
	def head(self):
		...
		t = 5, 5
		play8.screen64_printall(self.font[0], play8.bytes_shape(bytes.fromhex("70 43 49 42 20 42 6b 43 73 42 20 42 6d 43 4b"), 3), self.view, t)
		t = 8, 9
		...
		play8.screen64_printrow(self.font[2], s.encode('ascii'), self.view, t)
		t = 35, 0
		play8.screen64_printall(self.font[0], play8.bytes_shape(bytes.fromhex("61") * 25, 1), self.view, t)
		t = 36, 1
		s = "TIME"
		play8.screen64_printrow(self.font[2], s.encode('ascii'), self.view, t)
		t = 36, 5
		s = "PRESS SPACE"
		play8.screen64_printall(self.font[2], play8.bytes_shape(s.encode('ascii'), 1), self.view, t)
		b = True
		s = str()
		l = list("CODE")
		t = 36, 23
		while 1:
			if b:
				s2 = s.join(l)
				play8.screen64_printrow(self.font[2], s2.encode('ascii'), self.view, t)
				pygame.display.flip()
				b = False
	...
	def side(self, view, part, info):
		...
		i = 0
		s = "GET READY !"
		l = play8.bytes_shape(s.encode('ascii'), 1)
		s = "           "
		...
		while i < 8:
			for x in pygame.event.get():
				if x.type == pygame.QUIT:
					...
					i += 1
			if i % 2:
				play8.screen64_printall(self.font[2], l, view, t)
			else:
	...
	def tail(self):
		...
		while b:
			...
			t2 = 5, 20
			play8.screen64_printrow(self.font[2], s.encode('ascii'), self.view, t2)
			pygame.display.flip()
			i -= 5
			i %= 360
			c.tick(50)
	...
	def turn(self, view, part, info):
		...
		while b:
			...
			if pygame.sprite.spritecollideany(g.sprite, b2):
				if pygame.mixer:
					s = pygame.mixer.Sound(os.path.join(self.path, "res", "noise_5.1.wav"))
					s.play()
				b = False
			b2.move()
			view.fill(t)
	...
	def update_progress(self, view, progress):
		t = 36, 1
		s = "TIME"
		play8.screen64_progress(self.font[2], self.font[3], progress, s.encode('ascii'), view, t)
		return
...
class Ball(pygame.sprite.Sprite):
	...
	def __init__(self, view, font, spar):
		o = super()
		o.__init__()
		self.image = play8.screen64_make(font[0], play8.bytes_shape(bytes.fromhex("51"), 1))
		self.rect = self.image.get_rect()
		self.area = view.get_rect()
		if random.randint(0, 1):
			self.rect.left = random.randint(self.area.left, self.area.right - self.rect.width)
			...
			self.rect.top = random.randint(self.area.top, self.area.bottom - self.rect.height)
...
class Balls(pygame.sprite.Group):
	...
	def move(self):
		...
		for x in self.sprites():
			b.remove(x)
			l = pygame.sprite.spritecollide(x, b, False)
			if not l:
				x.safe = True
			for y in l:
				t = y.rect.centerx - x.rect.centerx, y.rect.centery - x.rect.centery
				a = numpy.array(t)
...
class Player(pygame.sprite.Sprite):
	...
	def move(self):
		...
		if i >= self.area.left and self.area.right >= self.rect.width + i and i2 >= self.area.top and self.area.bottom >= self.rect.height + i2:
			self.rect.left = i
			self.rect.top = i2
		return
